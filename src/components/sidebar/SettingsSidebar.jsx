    ...
    <div className={`sidebar-overlay ${isSidebarOpen ? "active" : ""}`}>
      <div className="sidebar" ref={sidebarRef}>
        ...
        <div className="bottom">
          ...
          <div className="pages-url-section">
            <h3 className="url-section-header">Layouts</h3>
            {layoutList.map((item, index) => (
              <div className={`single-page-btn-container ${activeLayout[`home${index + 1}`] ? 'active' : ''}`} key={item.id}>
                <Link to={item.url} className="page-image-btn">
                  <img src={item.imgSrc} alt={`home-${index + 1}`} role="button" onClick={() => handleItemClick(index)} />
                </Link>

                <Link to={item.url} className="page-name-text" role="button" onClick={() => handleItemClick(index)}>{item.title}</Link>
              </div>
            ))}
          </div>
        </div>
